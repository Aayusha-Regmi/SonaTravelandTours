...
const getAvailableSeats = async (busId, date, destination = "kathmandu") => {
  try {
    // Import apiConfig to get the proper base URL
    const { default: apiConfig } = await import('./api-config.js');
    const baseUrl = apiConfig.getBaseUrl();
    
    // Call the real backend API for seat details
    const response = await fetch(`${baseUrl}${import.meta.env.VITE_BUS_SEAT_DETAILS_ENDPOINT}`, {
      method: 'POST',
      headers: {
...
const getServiceCharge = async (amount, instrumentCode) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/payment/get-service-charge`, {
      method: 'POST',
      headers: headers,
      body: JSON.stringify(requestBody)
    });
...
const checkPaymentStatus = async (seatInfo, paymentInfo) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/payment/check-status`, {
      method: 'POST',
      headers: headers,
      body: JSON.stringify(requestBody)
    });
...
const getPaymentRedirectUrl = async (merchantTransactionId, processId, instrumentCode, successUrl, failureUrl) => {
  try {
    ...
    const url = `${baseUrl}/payment/onepg?${params.toString()}`;
...
const checkNPSTransactionStatus = async (merchantTxnId) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/payment/CheckTransactionStatus`, {
      method: 'POST',
      headers: headers,
      body: JSON.stringify(requestBody)
    });
...
const updatePaymentStatus = async (paymentInfo, status, seatInfo = null) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/seat/payment`, {
      method: 'POST',
      headers: headers,
      body: JSON.stringify(requestBody)
    });
...
const testAPIEndpoints = async () => {
  ...
  const endpoints = [
    ...
    },
    {
      ...
      url: `${baseUrl}/payment/initiate-payment`,
      method: 'POST',
      body: { amount: 100 }
    },
    {
      name: 'Payment Instruments',
      url: `${baseUrl}/payment/get-all-payment-instruments`,
      method: 'GET'
    }
  ];
