    ...
    <div className="bg-white rounded-xl border border-gray-100 p-0 md:p-8">
      ...
      <div className="flex flex-col items-center py-8 border-b border-gray-100">
        <div className="relative mb-4">
          ...
          />
          <button 
            className="absolute bottom-2 right-2 bg-red-500 text-white rounded-full p-2 shadow hover:bg-red-600 transition" 
            title="Edit Profile"
          >
        ...
        <p className="text-gray-500 text-sm mb-4">{profile?.email || 'user@example.com'}</p>
        <div className="flex gap-3 mb-2">
          <div className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-full text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3a1 1 0 011-1h6a1 1 0 011 1v4M8 7H5a1 1 0 00-1 1v8a1 1 0 001 1h14a1 1 0 001-1V8a1 1 0 00-1-1h-3M8 7h8" />
            </svg>
            Joined {profile?.createdOn ? new Date(profile.createdOn).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : 'Recently'}
          </div>
          <div className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-full text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            {profile?.totalBookings || bookings.length || 0} Booking{(profile?.totalBookings || bookings.length) === 1 ? '' : 's'}
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-gray-100 mt-6 mb-6 overflow-x-auto">
        {TABS.map(t => (
          <button
            key={t.key}
