...
const datepickerStyles = `
  ...
  /* Button hover effect */
  .datepicker-content button:hover:not(:disabled) {
  ...
  /* Better responsive design for datepicker layout */
  @media (max-width: 767px) {
    .datepicker-container .space-x-3 {
      flex-direction: column;
      gap: 1rem;
      width: 100%;
    }
    
    .datepicker-container .space-x-3 > div {
      width: 100% !important;
    }
  }
  
  /* Specific styles for search results page datepickers */
  .flex .datepicker-content {
    height: 100%;
    display: flex;
    flex-direction: column;
  }
  ...
  .datepicker-calendar {
    min-width: 300px;
    width: auto;
    box-shadow: 0 10px 25px -5px rgba(10, 99, 157, 0.2), 0 8px 10px -6px rgba(10, 99, 157, 0.1);
        ...
        <div key={`day-${day}`} className="h-[30px] flex items-center justify-center">
          <button
            ...
            className={`
              ...
              ${disabled 
                ? 'opacity-30 cursor-not-allowed hover:bg-transparent line-through hover:transform-none' 
    ...
    <div className={`relative datepicker-content ${className}`}>
      ...
      <div className="relative" ref={calendarRef}>
        <button
          ...
          style={{ 
            height: className?.includes('h-full') ? '100%' : '50px',
            minHeight: '50px'
          }}
        ...
        >
          ...
          <span className={`flex items-center justify-center h-8 w-8 rounded-full ${isOpen ? 'bg-[#0a639d]/10' : ''}`}>
            <svg 
              className={`w-5 h-5 text-[#0a639d] transition-all duration-300 ${isOpen ? 'scale-110' : ''}`}
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24" 
